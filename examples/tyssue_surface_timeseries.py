...
def history_to_4d_mesh(history):
    vertices = None
    faces = None
    colors = None
    for t in range(len(history)):
        ...
        if vertices is None:
            ...
            vertices = np.concatenate((vertices, np.ones((vertices.shape[0], 1,))*t), axis=1)
        else:
            these_vertices = mesh[0].copy()
            these_faces = mesh[1].copy()
            these_colors = mesh[2].copy()

            # Add time dimension to vertices
            these_vertices = np.concatenate((these_vertices, np.ones((these_vertices.shape[0], 1,))*t), axis=1)
            
            # Adjust vertex index
            these_faces += these_vertices.shape[0]
            
            vertices = np.concatenate((vertices, these_vertices), axis=0)
            faces = np.concatenate((faces, these_faces), axis=0)
            colors = np.concatenate((colors, these_colors), axis=0)
        print(f"mesh: ({mesh[0].shape}, {mesh[1].shape}, {mesh[2].shape})")
...
mesh4d = history_to_4d_mesh(history)

surface = viewer.add_surface(mesh4d, colormap='turbo', opacity=0.9, contrast_limits=[0, 1], name='tyssue')

(lambda m : print(f"mesh: ({m[0].shape}, {m[1].shape}, {m[2].shape})"))(mesh)
(lambda m : print(f"mesh: ({m[0].shape}, {m[1].shape}, {m[2].shape})"))(mesh4d)
