...
layer.contrast_limits = (0, 50_000)
...
@thread_worker
def animator():
    for x in range(0, empty.shape[0], chunk_size[0]):
        for y in range(0, empty.shape[1], chunk_size[1]):
            # time.sleep(0.1)
            z = 1000
            # result.data[x:(x + chunk_size[0]), y:(y + chunk_size[1]), z].compute()            
            yield (x, y, z, np.asarray(result.data[x:(x + chunk_size[0]), y:(y + chunk_size[1]), z].compute()))
